...
description: Handles bi-directional sync between local skills and GitHub. v1.0.15 fixes Issue #16 - upload_skill() auto-defends by calling change_classifier.classify_change() when classification is incomplete. Retains all v1.0.12 fixes (subdir prefix, action logic, frontmatter routing, CLI fallback, temp dir naming, CHANGELOG CI injection, LICENSE exclusion, local_dir derivation, expanduser, file filtering, _create_clean_temp_dir exclusion, per-file validation).
version: 1.0.15
...
auth_config:
  ...
  env_file_path: ../.env
file_mapping:
  local_path: scripts/sync_engine.py
...
class SyncEngine:
    ...
    def _record_pending_cleanup(self, path: Path, reason: str, auto_approved: bool = False) -> str:
        ...
        if not auto_approved:
            print("[CLEANUP_PENDING] User must run cleanup_pending() or manually delete. DO NOT auto-delete.")
        return str(manifest)
    ...
    def cleanup_pending(self, confirm: bool = False, dry_run: bool = True) -> dict:
        ...
        for manifest in manifests:
            ...
            if dry_run or not confirm:
                skipped.append({"path": str(path), "reason": record["reason"], "manifest": str(manifest), "action": "skipped (dry_run or confirm=False)"})
                continue
            if path.exists():
                try:
                    if path.is_file():
                        ...
                        shutil.rmtree(path)
                    deleted.append({"path": str(path), "reason": record["reason"], "manifest": str(manifest)})
                    archive_dir = self.pending_cleanup_dir / "executed"
                    archive_dir.mkdir(exist_ok=True)
                    manifest.rename(archive_dir / manifest.name)
                except Exception as e:
                    skipped.append({"path": str(path), "reason": f"ERROR: {e}", "manifest": str(manifest)})
            else:
                skipped.append({"path": str(path), "reason": "Path already missing", "manifest": str(manifest)})
        return {"deleted": deleted, "skipped": skipped, "dry_run": dry_run or not confirm, "total_pending": len(manifests)}
    ...
    def _validate_github_repository(self, frontmatter, source_file):
        ...
        if not repo_field:
            return {"valid": False, "owner": None, "repo": None, "error": "Missing github_repository in frontmatter: " + str(source_file)}
        repo_field = repo_field.strip().strip("/")
        parts = repo_field.split("/")
        if len(parts) != 2:
            return {"valid": False, "owner": None, "repo": None, "error": "Invalid github_repository format in " + str(source_file) + ". Must be owner/repo."}
        owner, repo = parts[0], parts[1]
        if not owner or not repo:
            return {"valid": False, "owner": None, "repo": None, "error": "Empty owner or repo in github_repository in " + str(source_file)}
    ...
    def compare_skill(self, skill_name, local_dir=None):
        ...
        if not modified and not local_only and not github_only:
            ...
            action = "diverged"
        return {
            "status": "ok", "owner": owner, "repo": repo, "local_dir": str(local_dir),
            "action": action, "identical_count": len([c for c in comparisons if c["status"] == "identical"]),
            "modified_count": len(modified), "local_only_count": len(local_only), "github_only_count": len(github_only),
            "modified_files": modified, "local_only_files": local_only, "github_only_files": github_only,
    ...
    def sync_skill(self, skill_name, local_dir=None, dry_run=True):
        ...
        if action in ("github_ahead", "diverged"):
            if dry_run:
                return {"status": "ok", "action": action, "dry_run": True, "message": f"GitHub is ahead. Would download {len(comparison['github_only_files'])} new files and overwrite {len(comparison['modified_files'])} modified files.", "github_only_files": comparison["github_only_files"], "modified_files": comparison["modified_files"]}
            ...
            return {"status": "ok", "action": "downloaded", "downloaded": result["downloaded"], "failed": result["failed"]}
    ...
    def upload_skill(self, skill_name, files, classification):
        # =====================================================================
        # FIX v1.0.15 Issue #16: Auto-defend incomplete classification.
        # If classification lacks change_classifier fields, auto-invoke wrapper.
        # =====================================================================
        if "bump_type" not in classification or "approval_required" not in classification:
            print("[UPLOAD] WARNING: classification incomplete (missing change_classifier fields). Auto-invoking classify_change()...")
            try:
                from scripts.change_classifier import classify_change
                classification = classify_change(classification)
                print("[UPLOAD] Auto-completed classification via classify_change()")
            except Exception as e:
                print(f"[UPLOAD] ERROR: classify_change() failed: {e}. Using safe fallback.")
                classification = {
                    **classification,
                    ...
                    "reason": classification.get("reason", "Auto fallback - classify_change failed"),
                }
        # =====================================================================

        # FIX v1.0.13 Issue #12: Safe access with defaults
        approval_required = classification.get("approval_required", False)
        if approval_required:
            self._move_to_pending(skill_name, files, classification)
            return {"status": "pending_approval", "reason": classification.get("reason", "Approval required")}
        ...
        commit_msg = template.format(
            bump_type=bump_type.upper(), skill_name=skill_name, new_version=new_version,
            agent_name=agent_name, model=model_name, summary=summary,
        )
        ...
        return {"status": "uploaded", "commit_message": commit_msg, "new_version": new_version, "repo_name_used": repo_name, "details": result}
    ...
    def _move_to_pending(self, skill_name, files, classification):
        ...
        meta = {"skill_name": skill_name, "classification": classification, "files": [str(f) for f in files], "timestamp": datetime.now(timezone.utc).isoformat()}
    ...
    def _log_rejected(self, skill_name, files, gate_result):
        ...
        with open(log_file, "w", encoding="utf-8") as f:
            json.dump({"skill_name": skill_name, "files": [str(f) for f in files], "reason": gate_result.get("error") or gate_result.get("reason"), "timestamp": datetime.now(timezone.utc).isoformat()}, f, indent=2, ensure_ascii=False)
    ...
    def _upload_via_cli(self, repo_name, files, commit_msg, skill_name=None):
        try:
            ...
            local_dir = Path(os.path.expanduser(str(self.cfg.user_skills_folder))) / (skill_name or repo_name)
            ...
            cmd = [sys.executable, str(cli_script), "--repo-name", repo_name, "--local-dir", str(clean_dir), "--repo-base-path", skill_dir_name, "--force"]
            result = subprocess.run(cmd, capture_output=True, text=True, cwd=str(dep_path))
            self._record_pending_cleanup(clean_dir, f"Upload temp dir for {skill_name or repo_name}. Safe to delete after confirming upload success.", auto_approved=False)
            print(f"[UPLOAD] Temp dir preserved for audit. Run cleanup_pending(confirm=True) to delete after verifying upload.")
            return {"method": "cli", "status": "success" if result.returncode == 0 else "error", "returncode": result.returncode, "stdout": result.stdout[:500] if result.stdout else "", "stderr": result.stderr[:500] if result.stderr else "", "clean_dir_used": str(clean_dir), "cleanup_status": "pending_user_confirmation"}
        except Exception as e:
    ...
    def sync_changelog(self, skill_name, local_dir=None):
        ...
        if not local_has_fm and remote_has_fm:
            ...
            return {"status": "synced", "action": "downloaded", "message": "CHANGELOG.md frontmatter synced from GitHub to local", "local_path": str(local_changelog)}
        elif local_has_fm and not remote_has_fm:
            return {"status": "pending_upload", "action": "local_ahead", "message": "Local CHANGELOG.md has frontmatter but remote does not. Run upload to sync."}
        elif local_content == remote_content:
            return {"status": "identical", "action": "identical", "message": "CHANGELOG.md is identical between local and remote."}
        else:
            return {"status": "diverged", "action": "diverged", "message": "CHANGELOG.md content differs. Manual review required."}
