...
description: "Quick diagnostic tool for daemon issues. Tests local_scanner frontmatter extraction, skill_installer filename cleaning, module cache status. Run after replacing any .py file to verify daemon will use the new version."
version: 1.0.0
github_repository: nervlin4444/ai.skills.incubation
target_branch: main
updated_at: 2026-05-23T21:40:00+08:00
fixes: []
auth_config:
  provider: github
  auth_method: token
...
def load_scanner_module():
    for key in list(sys.modules.keys()):
        if "local_scanner" in key or "skill_installer" in key:
            del sys.modules[key]
...
def check_module_cache():
    ...
    for name, mod in sys.modules.items():
        if "local_scanner" in name or "skill_installer" in name:
...
def main():
    print("=" * 70)
    print("DAEMON HEALTH CHECK v1.0.0")
    ...
    print("-" * 40)
    scanner, installer = load_scanner_module()
    print("  local_scanner: loaded")
    ...
    fm_results = test_frontmatter_extraction(scanner, test_cases)
    all_fm_ok = True
    for r in fm_results:
    ...
    if not all_fm_ok:
        issues.append("Frontmatter extraction FAILED - local_scanner needs fix")
    if not all_fn_ok:
