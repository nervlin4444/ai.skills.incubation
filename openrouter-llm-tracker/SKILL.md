---
title: "openrouter-llm-tracker — LLM SKILL.md"
name: "openrouter-llm-tracker"
description: "LLM execution guide. Fetch model registry, score models, test connectivity, generate official-format configs for OpenClaw and WorkBuddy, and smart-merge into existing agent configs."
version: "v1.2.2"
...
file_mapping:
  ...
  github_path: "openrouter-llm-tracker/SKILL.md"
---
...
1. Accept models list from S-001 and data_dir path.
2. Write llm.registry.json to data_dir with structure: {"models": [...], "meta": {"updated_at": "ISO-8601", "count": N}}
3. Write daily snapshot to data/llm.history/YYYY-MM-DD.json with identical structure.
4. If file already exists for today, overwrite.

### 3.3 S-003: Update Scores

1. Accept model_id, dimensions dict, and data_dir.
...
3. Load llm.scores.json from data_dir.
...
7. Write updated llm.scores.json back to data_dir.
...
|------|-------------|---------|
| List all models | python scripts/llm.registry.py "<data_dir>" --list | python scripts/llm.registry.py "openrouter-llm-tracker/assets" --list |
| List free models only | python scripts/llm.registry.py "<data_dir>" --free-only | python scripts/llm.registry.py "openrouter-llm-tracker/assets" --free-only |
| Filter by tier | python scripts/llm.registry.py "<data_dir>" --filter-tier <tier> | python scripts/llm.registry.py "openrouter-llm-tracker/assets" --filter-tier domain_expert |
| Filter by type | python scripts/llm.registry.py "<data_dir>" --filter-type <type> | python scripts/llm.registry.py "openrouter-llm-tracker/assets" --filter-type coding |
| Search by keyword | python scripts/llm.registry.py "<data_dir>" --search <keyword> | python scripts/llm.registry.py "openrouter-llm-tracker/assets" --search gemini |
| Get single model | python scripts/llm.registry.py "<data_dir>" --get <model_id> | python scripts/llm.registry.py "openrouter-llm-tracker/assets" --get openai/gpt-4o |
| Show statistics | python scripts/llm.registry.py "<data_dir>" --count | python scripts/llm.registry.py "openrouter-llm-tracker/assets" --count |
| Load snapshot | python scripts/llm.registry.py "<data_dir>" --snapshot YYYY-MM-DD | python scripts/llm.registry.py "openrouter-llm-tracker/assets" --snapshot 2026-05-11 --list |

Notes:
- Wrap data_dir in quotes if path contains spaces.
...
| Task | CLI Command | Example |
|------|-------------|---------|
| Top 20 leaderboard | python scripts/llm.scorer.py "<data_dir>" --leaderboard | python scripts/llm.scorer.py "openrouter-llm-tracker/assets" --leaderboard |
| Top N leaderboard | python scripts/llm.scorer.py "<data_dir>" --leaderboard N | python scripts/llm.scorer.py "openrouter-llm-tracker/assets" --leaderboard 10 |
...
| File | Path |
|------|------|
| Registry | openrouter-llm-tracker/data/llm.registry.json |
| Scores | openrouter-llm-tracker/data/llm.scores.json |
| History | openrouter-llm-tracker/data/llm.history/YYYY-MM-DD.json |
