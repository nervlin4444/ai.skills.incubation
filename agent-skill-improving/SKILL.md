...
target_branch: "main"
updated_at: "2026-05-22T16:24:17+08:00"
auth_config:
  provider: "github"
  auth_method: "token"
...
# agent-skill-improving v1.2.5 — 執行指令
...
## Guardian Pattern 執行流程（v1.2.5 強制）

```
Agent 需要創建/修改任何 .md / .py / .json / .html 文件
    ↓
Step 1: 調用 skill_files_designer.py 生成 frontmatter
    ↓
...
主人說 YES 後，按以下順序執行：
 ...
 2. 生成 Patch → PATCH_{old}_to_{new}.md
 3. 應用 Patch → skill_patch_validator.py（replace 優先，3次失敗降級 write）
 4. 更新版本號 → 語義化版本（Hotfix=修訂+1 / Minor=次版本+1 / Major=主版本+1）
 5. 驗證合規 → skill_integrity_checker.py --strict
 6. 驗證跨平台 → 檢查 Windows/Linux/macOS 兼容性
 7. 驗證長內容 → 測試 >500 字符內容是否能正確傳遞
 8. 驗證臨時文件 → 確認 JSON 文件生成與清理機制
 9. 記錄歷史 → SKILL_CORRECTION.md
 10. 如需報告 Issue → skill_issue_reporter.py --from-stdin
 11. 通知 skill-acquiring → 新版本可用
