...
description: 技能改進操作手冊。涵蓋Guardian Pattern事前強制架構、腳本更名規範、Patch應用策略分級、強制合規驗證、跨平台兼容檢查、長內容處理檢查、臨時文件管理檢查、標準Issue報告生成。融入conversation_append.py v1.4.0長文件經驗及github-skill-organizer v1.0.4-v1.0.12教訓。
version: v1.2.5
github_repository: nervlin4444/ai.skills.incubation
target_branch: main
updated_at: 2026-05-22T16:22:27+08:00
auth_config:
  provider: github
  auth_method: token
...
> 當前版本：v1.2.5
> 配套文件：SKILL.md v1.2.5 + CONTRIBUTING.md v1.2.5 + SOUL.md v5.0 + IDENTITY.md v5.0
> **核心變更：v1.2.5 引入 Guardian Pattern 事前強制架構，腳本全面更名，新增標準 Issue 報告器**
...
| 角色 | 說明 |
|------|------|
| Agent | 執行改進操作（Patch 應用、合規驗證、Issue 報告生成） |
...
│ 腳本：skill_issue_reporter.py           │
│ 作用：區分 [FRAMEWORK] / [RUNTIME]      │
│        / [AGENT-BUG]，強制規範輸出      │
│ 結果：只有 [FRAMEWORK] 上報主人         │
└─────────────────────────────────────────┘
 ↓
GitHub Issue → 修復 → Commit Fixes #N → 自動關閉 → 本地驗證
```
...
| 舊名稱 | 新名稱 | 功能 |
|--------|--------|------|
| skill_validate.py | **skill_integrity_checker.py** | 合規檢查器（28+ 項檢查 + 8 項紅線） |
...
| （新增） | **skill_issue_reporter.py** | Issue 報告器（強制按 CONTRIBUTING.md 格式輸出） |
...
**用法**：
```bash
python skill_folder_designer.py --name "new-skill" --description "..."
```

### 4.5 skill_issue_reporter.py（Issue 報告器）— v1.2.5 新增

**功能**：強制按 CONTRIBUTING.md v1.2.5 規範生成標準 Issue 報告。
...
**觸發時機**：
- `skill_integrity_checker.py` 檢查失敗時（可選自動觸發）
- Agent 發現無法自行修復的 [RUNTIME] 問題時
- 主人要求創建 Issue 時
...
| **v1.2.5** | **2026-05-22** | **引入 Guardian Pattern 事前強制架構、腳本全面更名、github_path 前導 "/" 防禦處理、Issue 自動閉環機制、新增 skill_issue_reporter.py 標準 Issue 報告器** |
...
**Q4：如何做到 Issue 自動閉環？**
A：Agent 發現 [RUNTIME] 問題 → 調用 skill_issue_reporter.py 生成標準報告 → 主人審核創建 Issue → Agent 自行修復 → Commit message 包含 `Fixes #{issue_number}` → GitHub 自動關閉 Issue → Agent 本地驗證。只有 [FRAMEWORK] 問題需要主人介入決策。

**Q5：為什麼 Issue 必須通過腳本生成？**
A：如果讓 Agent 自由撰寫，不同 LLM 的輸出格式、詳細程度、分類判斷都不一致，導致主人審核成本高、Kimi 分析困難。通過腳本強制鎖定格式，確保任何 LLM 輸出都完全一致。
...
| 文件 | 說明 |
|------|------|
| SKILL.md | LLM 執行指令（含 LOCK-009~013） |
| CONTRIBUTING.md | 標準 Issue 報告格式（結構化模板 + JSON） |
| SOUL.md v5.0 | 人格定義 |
| IDENTITY.md v5.0 | 身份定義 |
| scripts/USAGE.md | 腳本使用教程（含 skill_issue_reporter.py 用法） |
...
| scripts/skill_issue_reporter.py | Issue 報告器 |
